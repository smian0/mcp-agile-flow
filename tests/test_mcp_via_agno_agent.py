...
async def _test_project_settings(mcp_tools):
    ...
    reliability_result: Optional[ReliabilityResult] = reliability_eval.run(print_results=True)
    ...
    try:
        ...
        assert os.path.exists(os.path.join(project_path, "ai-kngr")), "Knowledge graph directory does not exist"
        assert os.path.exists(os.path.join(project_path, "ai-docs")) or os.makedirs(os.path.join(project_path, "ai-docs")), "AI docs directory created"
...
async def _test_initialize_ide_rules(mcp_tools):
    ...
    agent = create_agent(
        tools=[mcp_tools, FileTools(Path(project_path)), TextReader(Path(project_path))],
        debug=True
    )
...
async def _test_initialize_ide_rules_reliability(mcp_tools):
    ...
    for file_path in glob.glob(os.path.join(project_path, ".clinerules*")):
        try:
            ...
            print(f"Backed up existing rules file {file_path} to {file_path}.old_backup")
...
async def _test_knowledge_graph_creation(mcp_tools):
    ...
    try:
        ...
        assert os.path.exists(kngr_dir), f"Knowledge graph directory '{kngr_dir}' does not exist"
        
        # Read the graph to verify entities and relations
        response: RunResponse = await agent.arun("Show me the current knowledge graph structure")
...
async def _test_fastapi_project_knowledge_graph(mcp_tools):
    ...
    fastapi_project_path = Path("/Users/smian/development/mcp-agile-flow/tests/full-stack-fastapi-sample-project")
    ...
    try:
        ...
        # Read the graph to verify entities and relations
        response: RunResponse = await agent.arun("Show me the complete knowledge graph structure with all entities, relations, and observations")
...
test_initialize_ide_rules_reliability = run_with_mcp_tools(_test_initialize_ide_rules_reliability)
test_knowledge_graph_creation = run_with_mcp_tools(_test_knowledge_graph_creation)
test_fastapi_project_knowledge_graph = run_with_mcp_tools(_test_fastapi_project_knowledge_graph)
