...
def test_initialize_ide_rules_with_root_path():
    ...
    result = asyncio.run(
        handle_call_tool("initialize-ide-rules", {"ide": "cursor", "project_path": "/"})
    )
...
def test_initialize_ide_rules_with_env_root_path():
    ...
    logger.info(
        "Testing initialize-ide-rules safety with environment variable pointing to root..."
    )
    ...
    try:
        ...
        result = asyncio.run(
            handle_call_tool("initialize-ide-rules", {"ide": "cursor"})
        )
...
def test_get_project_settings_tool_with_proposed_path():
    ...
    result = asyncio.run(
        handle_call_tool("get-project-settings", {"proposed_path": "/"})
    )
    assert result[0].type == "text"
    ...
    assert (
        "root" in response_root["source"].lower()
        or "fallback" in response_root["source"].lower()
    )
    ...
    if parent_dir != "/" and os.access(parent_dir, os.W_OK):
        result = asyncio.run(
            handle_call_tool("get-project-settings", {"proposed_path": parent_dir})
        )
        assert result[0].type == "text"
        ...
        assert response_parent["is_writable"] is True

    # Test with a non-existent path
    with tempfile.NamedTemporaryFile() as temp_file:
        # Create a path that definitely doesn't exist
        non_existent_path = os.path.join(
            os.path.dirname(temp_file.name), f"non_existent_dir_{uuid.uuid4().hex}"
        )

        # Test with non-existent path
        result = asyncio.run(
            handle_call_tool(
                "get-project-settings", {"proposed_path": non_existent_path}
            )
        )
        assert result[0].type == "text"
        ...
        assert response_real["exists"] is True
        assert response_real["is_writable"] is True
