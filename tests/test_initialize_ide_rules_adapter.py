"""
Tests for the initialize-ide-rules tool functionality using the adapter.

These tests verify that the initialize-ide-rules tool correctly initializes rules
for different IDEs using the adapter that can switch between server and FastMCP implementations.
"""
...
def test_initialize_ide_rules_with_custom_path(tmp_path):
    ...
    save_test_output("initialize_ide_rules_response", result)
...
def test_initialize_ide_rules_with_root_path():
    ...
    result = asyncio.run(
        call_tool("initialize-ide-rules", {"project_path": "/", "ide": "cursor"})
    )
    ...
    # Save the response to the test_outputs directory
    save_test_output("initialize_ide_rules_with_root_path", result)

    # The rules should be created in the current directory
    current_dir = os.getcwd()

    # Check if the path in the response is not the root directory
    assert result["rules_directory"].startswith(current_dir)
    assert not result["rules_directory"].startswith("/.")
...
def test_initialize_ide_rules_cline(tmp_path):
    ...
    # Save the response to the test_outputs directory
    save_test_output("initialize_ide_rules_cline_response", result)

    # Verify response contains the correct rule file path and no directory
    assert result.get("rules_file") == str(cline_rules_file)
    assert result.get("rules_directory") is None
    assert "initialized_windsurf" not in result 