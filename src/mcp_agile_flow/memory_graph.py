...
class KnowledgeGraphManager:
    ...
    def _determine_graph_path(self) -> str:
        ...
        try:
            ...
            if (
                "knowledge_graph_directory" in settings
                and settings["knowledge_graph_directory"]
            ):
    ...
    def _ensure_directory_exists(self) -> None:
        ...
        if os.path.exists(directory):
            ...
            if not os.access(directory, os.W_OK):
                self.logger.warning(
                    f"Directory exists but is not writable: {directory}"
                )
    ...
    def _detect_project_type(self, project_path: str) -> Tuple[str, Dict[str, Any]]:
        ...
        try:
            ...
            for lang, files in software_indicators.items():
                for file in files:
                    if os.path.exists(os.path.join(project_path, file)) or any(
                        f.endswith(file)
    ...
    def _create_new_graph(self) -> KnowledgeGraph:
        ...
        if (
            project_path.endswith("ai-kngr")
            or project_path.endswith(".kg")
            or project_path.endswith(".knowledge")
        ):
        ...
        return KnowledgeGraph(
            project_type=project_type, project_metadata=project_metadata
        )
    ...
    def _load_graph(self) -> KnowledgeGraph:
        ...
        if os.path.exists(self.graph_path):
            try:
                ...
                self.logger.info(
                    f"Successfully loaded graph with {len(entities)} entities and {len(relations)} relations"
                )
                return graph
            except json.JSONDecodeError as e:
                self.logger.error(
                    f"JSON decode error loading graph from {self.graph_path}: {e}"
                )
                self._backup_corrupted_file("json_error")
            except KeyError as e:
                self.logger.error(
                    f"Key error loading graph (missing required field): {e}"
                )
                ...
                self.logger.error(
                    f"I/O error loading graph from {self.graph_path}: {e}"
                )
                # No backup needed for I/O errors, the file is likely just inaccessible
        else:
            self.logger.info(
                f"Knowledge graph file does not exist at {self.graph_path}, creating new graph"
            )
    ...
    def _backup_corrupted_file(self, error_type: str) -> None:
        ...
        try:
            if os.path.exists(self.graph_path):
                ...
                self.logger.info(
                    f"Created backup of corrupted knowledge graph at {backup_path}"
                )
                ...
                try:
                    ...
                    self.logger.warning(
                        f"Could not read content of corrupted file: {read_error}"
                    )
        except Exception as e:
        ...
        for entity in self.graph.entities:
            ...
            if self.graph.project_type == "software":
                ...
                for obs in entity.observations[
                    :5
                ]:  # Limit to 5 observations to keep diagram clean
                ...
                if entity.entity_type == "person":
                    lines.append(
                        f"style {safe_id} fill:#f9f,stroke:#333,stroke-width:2px"
                    )
                elif entity.entity_type == "concept":
                    lines.append(
                        f"style {safe_id} fill:#bbf,stroke:#333,stroke-width:1px"
                    )
                elif entity.entity_type == "file":
                    lines.append(
                        f"style {safe_id} fill:#bfb,stroke:#333,stroke-width:1px"
                    )
                elif entity.entity_type == "task":
                    lines.append(
                        f"style {safe_id} fill:#fbf,stroke:#333,stroke-width:1px"
                    )
    ...
    def create_entities(self, entities: List[Dict[str, Any]]) -> List[Entity]:
        ...
        for entity_data in entities:
            ...
            existing_entity = next(
                (e for e in self.graph.entities if e.name == name), None
            )
            if existing_entity:
        ...
        for relation_data in relations:
            ...
            if not from_exists or not to_exists:
                ...
                if not from_exists:
                    self.graph.entities.append(
                        Entity(name=from_entity, entity_type="unknown")
                    )
                if not to_exists:
                    self.graph.entities.append(
                        Entity(name=to_entity, entity_type="unknown")
                    )
            ...
            existing_relation = next(
                (
                    ...
                    if r.from_entity == from_entity
                    and r.to_entity == to_entity
    ...
    def add_observations(
        ...
        for observation_data in observations:
            ...
            # Find the entity
            entity = next(
                (e for e in self.graph.entities if e.name == entity_name), None
            )
            if entity:
            ...
            result.append(
                {"entityName": entity_name, "addedObservations": added_observations}
            )
    ...
    def delete_entities(self, entity_names: List[str]) -> KnowledgeGraph:
        ...
        # Remove the entities
        self.graph.entities = [
            e for e in self.graph.entities if e.name not in entity_names
        ]
    ...
    def update_markdown_with_mermaid(self) -> str:
        ...
        try:
            ...
            print(
                f"Warning: Could not save Markdown file with Mermaid diagram to {md_path}: {e}"
            )
    ...
    def search_nodes(self, query: str) -> KnowledgeGraph:
        ...
        for entity in self.graph.entities:
            if (
                query in entity.name.lower()
                or query in entity.entity_type.lower()
                or any(query in obs.lower() for obs in entity.observations)
            ):
        ...
        for relation in self.graph.relations:
            if (
                relation.from_entity in matching_entities
                or relation.to_entity in matching_entities
            ):
                result_graph.relations.append(
                    Relation(
                        from_entity=relation.from_entity,
                        to_entity=relation.to_entity,
                        relation_type=relation.relation_type,
