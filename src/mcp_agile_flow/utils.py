...
def get_project_settings(proposed_path: str = None) -> Dict[str, Any]:
    ...
    if is_root:
        if current_directory == '/':
            logger.warning(f"Root path specified and current directory is also root. Using home directory instead.")
            project_path = home_directory
            source = "home directory (safety fallback from root)"
            is_root = False
        else:
            logger.warning(f"Root path specified. Using current directory for safety: {current_directory}")
            project_path = current_directory
    ...
    if not exists:
        if current_directory == '/':
            logger.warning(f"Path doesn't exist: {project_path} and current directory is root. Using home directory.")
            project_path = home_directory
            source = "home directory (fallback from non-existent path)"
        else:
            logger.warning(f"Path doesn't exist: {project_path}. Using current directory: {current_directory}")
            project_path = current_directory
    ...
    if not is_writable:
        if current_directory == '/':
            logger.warning(f"Path not writable: {project_path} and current directory is root. Using home directory.")
            project_path = home_directory
            source = "home directory (fallback from non-writable path)"
        else:
            logger.warning(f"Path not writable: {project_path}. Using current directory: {current_directory}")
            project_path = current_directory
