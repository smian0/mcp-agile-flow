...
async def handle_call_tool(
    ...
    try:
        ...
        # Memory graph endpoints are now handled by memory graph server
        elif name in ["create-entities", "create-relations", "add-observations", 
                     "delete-entities", "delete-observations", "delete-relations", 
                     "search-nodes", "open-nodes"]:
            logger.info(f"{name} functionality has been moved to memory graph server")
            return [
                create_text_response(
                    json.dumps({
                        "success": False,
                        "message": f"The {name} functionality has been moved to the memory graph MCP server."
                    }),
                    is_error=True
                )
            ]
