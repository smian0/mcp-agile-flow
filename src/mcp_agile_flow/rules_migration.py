...
def read_mdc_files(rules_dir: Path, specific_file: str = None) -> List[Tuple[str, str]]:
    ...
    if specific_file:
        file_name = specific_file if specific_file.endswith('.mdc') else f"{specific_file}.mdc"
        if file_name in mdc_files:
            mdc_files = [file_name]
        else:
            logger.error(f"Specified file '{specific_file}' not found in Cursor rules directory.")
...
def smart_truncate(content: str, max_length: int) -> str:
    ...
    sentence_end_candidates = [m.end() for m in re.finditer(r'[.!?]\s+', content[:target_length])]
...
def migrate_cursor_to_windsurf(
    ...
    try:
        ...
        if content_length > 6000:
            logger.warning(f"Content length ({content_length} chars) exceeds Windsurf's 6000 character limit")
